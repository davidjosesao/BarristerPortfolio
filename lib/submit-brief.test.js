...
jest.mock('@google/generative-ai', () => {
  ...
  MockGoogleGenerativeAI._mockGenerateContent = mockGenerateContent;
...
jest.mock('@supabase/supabase-js', () => {
  ...
  mockCreateClient._mockInsert = mockInsert;
...
const mockGenerateContent = GoogleGenerativeAI._mockGenerateContent;
const { createClient } = require('@supabase/supabase-js');
const mockInsert = createClient._mockInsert;
...
describe('generateSummary', () => {
  ...
  test('returns Gemini response text on success', async () => {
    mockGenerateContent.mockResolvedValue({ response: { text: () => '— Parties: Smith v Jones' } });
    const result = await generateSummary(BRIEF_DATA);
    expect(result).toBe('— Parties: Smith v Jones');
    expect(mockGenerateContent).toHaveBeenCalledWith(expect.stringContaining('Parties: Smith v Jones'));
  });
  ...
  test('uses "Not set" for missing hearingDate', async () => {
    mockGenerateContent.mockResolvedValue({ response: { text: () => '— summary' } });
    const data = { ...BRIEF_DATA };
    delete data.hearingDate;
    await generateSummary(data);
    const prompt = mockGenerateContent.mock.calls[0][0];
    expect(prompt).toContain('Hearing date: Not set');
  });

  test('omits firm name from prompt when not provided', async () => {
    mockGenerateContent.mockResolvedValue({ response: { text: () => '— summary' } });
    const data = { ...BRIEF_DATA };
    delete data.firmName;
    await generateSummary(data);
    const prompt = mockGenerateContent.mock.calls[0][0];
    expect(prompt).toContain('Submitter: Jane Smith\n');
  });
});
...
describe('handler', () => {
  ...
  test('returns 200 even when Gemini API fails (non-fatal)', async () => {
  ...
  test('returns 500 when saving the brief to Supabase fails', async () => {
    mockInsert.mockResolvedValue({ error: { message: 'insert failed', code: '23502' } });
    const res = makeRes();
    await handler(makeReq(), res);
